...
struct HomePage: View {
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
            ZStack(alignment: .bottomTrailing) {
                ...
                VStack(spacing: 16) {
                    ...
                    if isLoading {
                        ...
                        VStack(spacing: 12) {
                            Image(systemName: currentUserRole == "collocator" ? "house.badge.plus" : "magnifyingglass")
                                .font(.largeTitle)
                                .foregroundColor(AppTheme.textSecondary)
                            
                            if currentUserRole == "collocator" {
                                Text("Gérez vos annonces")
                                    .font(.headline)
                                Text("Ajoutez votre première annonce avec le bouton +")
                                    .font(.subheadline)
                                    .foregroundColor(AppTheme.textSecondary)
                            } else {
            ...
            .navigationTitle(currentUserRole == "collocator" ? "Espace Colocataire" : "Accueil")
            .task {
                await loadProperties()
                // Charger aussi les visites pour avoir les avis
