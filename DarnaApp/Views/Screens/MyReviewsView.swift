...
struct MyReviewsView: View {
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // Content
                    if isLoading && viewModel.allEnrichedReviews.isEmpty {
                        LoadingView()
                            .padding(.top, 60)
                    } else if viewModel.allEnrichedReviews.isEmpty {
                        emptyState
                            .padding(.top, 60)
                    } else {
                        // Statistics Card
                        ...
                        LazyVStack(spacing: 16) {
                            ForEach(viewModel.allEnrichedReviews) { enriched in
    ...
    private var emptyState: some View {
        VStack(spacing: 20) {
            ...
            Text("Aucune évaluation trouvée")
                .font(.system(size: 18, weight: .semibold))
                .foregroundColor(.primary)
            
            Text("Il n'y a pas encore d'avis sur la plateforme.")
                .font(.system(size: 14))
                .foregroundColor(.secondary)
                .multilineTextAlignment(.center)
    ...
    private func loadReviews() async {
        // Mode "Mur des avis" : Tout le monde voit tout
        await viewModel.loadGlobalReviews()
    }
